...
class PointCloudDataset(Dataset):
    def __init__(self, root, dataset, split, resolution, transform=None):
        ...
        for fn in tqdm(os.listdir(self.pcl_dir), desc="Loading"):
            ...
            # Clean name logic (Adjust based on your file naming)
            raw_name = fn[:-4] 
            clean_name = raw_name.split('_')[0] # Heuristic: "chair_01" -> "chair"
            self.pointcloud_names.append(clean_name)

            # Computing and storing semantic embedding
            if clean_name not in self.name_to_emb:
                with torch.no_grad():
                    text = clip.tokenize([f"a 3D point cloud of a {clean_name}"]).to(self.device)
                    emb = self.clip_model.encode_text(text)
                    self.name_to_emb[clean_name] = emb.cpu().squeeze(0).float()
    ...
    def __getitem__(self, idx):
        ...
        ## adding semantic embedding
        semantic_emb = self.name_to_emb[name] 

        data = {
            "pcl_clean": self.pointclouds[idx].clone(), 
            "name": name,
            "semantic_emb": semantic_emb # Add this
        }
...
def create_collate_fn(aligner):
    def align_patches(data):
        ...
        for d in data:
            ...
            sem_list = [d["semantic_emb"] for d in data]
            aligned_data["semantic_emb"] = torch.stack(sem_list)
        return aligned_data
...
class PairedPatchDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.on_the_fly:
            ...
            # Grab embedding from inner dataset
            semantic_emb = pcl_data["semantic_emb"] 
            ...
            # If using offline patches, you need to handle semantic_emb storage/retrieval there too.
            # Assuming on_the_fly=True for now based on your config.
            pass 
        ...
        new_data = {
            ...
            "center": center,
            "scale": scale,
            "semantic_emb": semantic_emb # Pass it out!
        }
        return new_data
